...
async function up(knx) {
  ...
  await knex.schema.createTable('properties', property => {
    property.increments();
  ...
  await knex.schema.createTable('property_docs', doc => {
    doc.increments();
  ...
  await knex.schema.createTable('property_notes', note => {
    note.increments();
  ...
  await knex.schema.createTable('tenants', tenant => {
    tenant.increments();
  ...
  await knex.schema.createTable('tenant_docs', doc => {
    doc.increments();
  ...
  await knex.schema.createTable('tenant_notes', note => {
    note.increments();
  ...
  await knex.schema.createTable('contracts', contract => {
    contract.increments();
  ...
  await knex.schema.createTable('contract_docs', doc => {
    doc.increments();
  ...
  await knex.schema.createTable('contract_notes', note => {
    note.increments();
  ...
  await knex.schema.createTable('properties_to_contract', pivot => {
    pivot.increments();
  ...
  await knex.schema.createTable('contracts_dd', cdd => {
    cdd.increments();
    cdd.integer('ddId').notNullable();
    cdd.foreign('ddId').references('id').inTable('direct_debits').onDelete('cascade');
    cdd.integer('contractId').notNullable();
    cdd.foreign('contractId').references('id').inTable('contracts').onDelete('cascade');
  }).then().catch((e) => {console.log(e)});
  
  await knex.schema.createTable('incomes', income => {
    income.increments();
  ...
  await knex.schema.createTable('income_docs', doc => {
    doc.increments();
  ...
  await knex.schema.createTable('outgoings', outgoing => {
    outgoing.increments();
  ...
  await knex.schema.createTable('outgoing_docs', doc => {
    doc.increments();
  ...
  await knex.schema.createTable('tasks', tasks => {
    ...
    tasks.enu('type', ['income', 'outgoing', 'directDebit']);
  ...
  await knex.schema.createTable('tasks_calendar', task => {
    task.increments();
    task.integer('taskId');
    task.foreign('taskId').references('id').inTable('tasks').onDelete('cascade');
    task.date('date').notNullable();
  }).then().catch((e) => {console.log(e)});
  
  await knex.schema.createTable('notifications', not => {
    not.increments();
    not.integer('userId');
